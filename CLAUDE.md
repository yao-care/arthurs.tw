...
AI 網站銷售漏斗站。**核心商品＝一次性顧問服務**（含用 AI 建站＋交接＋帶你上手；**沒有月費**）。定位（2026-07-26 老闆改口徑，`site.ts` 的 `POSITIONING`）：「AI 這波，人人都用得上。你看不懂、沒空管網站也沒關係，把網站交給我。」原本的「大家都能享受 AI 紅利」被文案守門判為浮誇口號而換掉（2026-08-01 實查：全站已無「AI 紅利」字樣，原註記說 about/service/index 等處仍留舊說法已過時）——**不是**便宜的按頁建站/網頁外包，別把它寫成「建站費/按頁數計費」。**網站本身即服務的活體示範**（用同一套方式建置、由 AI 持續協助更新內容）。
...
- **sitemap `lastmod`（2026-08-02 加，別拿掉）**：`astro.config.mjs` 的 `serialize` 會逐頁寫真實 `lastmod`——內容頁取 frontmatter 的 `updated`（缺則 `created`），靜態頁取該頁原始檔最後一次 commit 日期，查不到就不給。**鐵律：不得整份塞 `new Date()`**，那等於宣稱 53 頁今天全改過，Google 會折扣整份 sitemap。緣由：實測 13 天，Google 爬過的頁 100% 收錄（歷來 0 筆 `Crawled - not indexed`），卡住的 20 頁全部 `lastCrawlTime=null`＝根本還沒被爬，瓶頸是爬取配額不是內容品質；而原本整份 sitemap 沒有任何 `lastmod`，只有 Google 明說不看的 `changefreq`/`priority`。**推論：新頁產出速度（2-4 頁/天）遠高於爬取速度（0.67 頁/天）時，加頁只會讓佇列變長**，該做的是站外訊號而非再寫內容。
  - ⚠️ **2026-08-03 校正：上面那句「瓶頸是爬取配額、只能等」講得太滿**。當天查 GSC sitemaps API 發現 `lastDownloaded` 停在 07-28、Google 手上只有 **41 個網址**，而線上 sitemap 已有 **73 個且逐頁有 lastmod**——**32 頁從沒被告知過，08-02 加的 lastmod 一次都沒被讀到**。也就是有一段不是配額，是我們沒送。**GSC API 沒有「請求建立索引」端點**（UI 那顆按鈕沒有 API），唯一正規手段是 `sitemaps.submit`，當天已補進 `lib/google.mjs` 並由 `seo-collect` 每天自動判斷過期才重送（詳見 `.claude/ops/seo-ops.md` 同名章節）。**往後看到「未收錄」先查 `lastDownloaded` 與線上網址數對不對得上，再談配額。**
  - 📌 **2026-08-04 起的作法：缺口優先補進既有頁，不是開新頁**。當日 71 個追蹤網址仍有 40 個 `lastCrawlTime=null`。該日盤出 7 個內容缺口（來源＝PTT 五個相關板 521 筆討論串標題，非 Google 搜尋建議），**只開 2 個新網址**（`website-or-social-media`／`seo-or-ads`），其餘 5 個（公司信箱與 DNS／WordPress 與 Wix／網站被駭與託管商出事／一頁式與手機版該不該加價／「侵權」用字）全部補進既有文章，不增 URL。**佇列還深的時候，補既有頁比開新頁划算**，因為既有頁多半已經被爬過，改了 `updated` 就會帶新的 `lastmod` 進 sitemap。
